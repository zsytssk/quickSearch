...
chrome.storage.sync.get("quickSearch", function (r) {
  ...
  if (!r.quickSearch) {
    ...
    islastUpdate =
      Date.parse(r.quickSearch.lastUpdate) >= Date.parse(lastUpdate);
  }
...
httpRequest(
  chrome.extension.getURL("content_scripts/quickSearch.html"),
  function (data) {
...
function calcbsboxheight() {
  bscon.style.cssText =
    "padding-top: 40px; position: absolute; left: 0; top: 0; width: 100%;";
  bscon.style.height = window.innerHeight - 40 + "px";
}
...
function changeSearchEngine() {
  ...
  quickSearch.curIndex =
    quickSearch.curIndex + 1 < quickSearch.List.length
      ? quickSearch.curIndex + 1
      : 0;
  searchurl = quickSearch.List[quickSearch.curIndex].url;
  chrome.storage.sync.set(
    {
